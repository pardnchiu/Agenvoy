...
七個後端統一於單一介面 — 專屬 Planner LLM 自動為每個請求挑選最適 Provider，並自動裁剪 Context Window。

<details>
<summary>詳細說明</summary>

GitHub Copilot、Claude、OpenAI、OpenAI Codex（OAuth）、Gemini、Nvidia NIM，以及任意 OpenAI 相容端點（Compat/Ollama）— 統一於 `Agent` 介面。Planner LLM 自動選出最適 Provider，token-budget 裁剪防止 context window 溢出，各 Provider 支援獨立設定的推理層級（Reasoning Level）。
...
- **v0.17.4** — 新增 OpenAI Codex 作為獨立 OAuth Provider（Device Code Flow 含自動刷新，預設模型 `gpt-5.3-codex`）。新增 `read_image` 工具，讀取本地圖片檔案並回傳 base64 data URL — 支援 JPEG、PNG、GIF、WebP，最大 10 MB，統一 decode 後 re-encode 為 JPEG。恢復 Yahoo Finance 原生 Go 工具（`fetch_yahoo_finance`），以並行雙端點抓取（query1/query2）取代已移除的 JSON API Extension。修正 CLI 摘要生成可靠性，改為先完成摘要流程再發送最終完成訊號。修正 `search_history` 與 `fetch_google_rss` 在模型誤傳 `query` 而非 `keyword` 時的 fallback 行為。修正工具執行錯誤未帶 `Err` 時的 event logging crash。修正 `discussion_log` 依最早上下文訊息過濾過舊項目的問題。修正 scheduler task 與 cron 流程的 nil-safe 問題並補齊 CRUD。更新 system prompt 新聞查詢強制 fallback 順序 `1h → 24h → 7d → search_web`。重構摘要擷取為獨立流程，簡化 Provider 初始化路徑。
