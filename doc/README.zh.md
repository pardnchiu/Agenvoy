...
橫切關注的工具套件，提供 Agenvoy 建構其上的 HTTP、瀏覽器、keychain 與通用基礎元件。`go-utils/http` 提供泛型 `GET[T]` / `POST[T]` / `PUT[T]` / `PATCH[T]` / `DELETE[T]` client，貫穿所有 provider（`claude` / `openai` / `copilot` / `compat` / `gemini` / `nvidia`）與原生 API 工具（`yahooFinance` / `youtube` / `googleRSS` / `searchWeb`）。`go-utils/rod` 掌管 `fetch_page` 背後的 headless Chrome 堆疊 — stealth JS、listener-settle 偵測、viewport 處理、typed `FetchError{Status}`、`KeepLinks`、以及具 idle-TTL 驅逐的 process-singleton browser。`go-utils/filesystem/keychain` 驅動憑證儲存（macOS `security` / Linux `secret-tool` / 檔案 fallback），`go-utils/utils.UUID()` 則提供共用 ID 生成器。
