...
Two layers; session overrides global. See the MCP Integration page for full schema and `${VAR}` expansion behavior.
...
Enabled state is `kuradb_enabled: bool` in config.json. Toggle via `/feature kuradb` in the TUI (no CLI subcommand — install.sh + sudo need a real TTY). See the KuraDB RAG page for full lifecycle.
