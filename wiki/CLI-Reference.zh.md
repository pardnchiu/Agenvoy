...
| `/dispatcher` | 從 `cfg.Models` 挑 dispatcher model。 |
...
| `/telegram [enable\|disable]` | 啟用／停用 Telegram bot（與 `/discord` 同模式的 in-TUI popup chain；首次與 bot 對話的 chat 必須通過 in-chat 驗證碼，授權清單存於 `~/.config/agenvoy/.telegram`）。 |
