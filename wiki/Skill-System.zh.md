...
`MatchSkillCall` 命中時，agenvoy 合成 `run_skill` 的 `tool_call` 與對應 `tool_result`（內含 skill body）直接注入 `ToolHistories` —— 與自然語言啟用路徑 byte-identical，保留 prefix cache。
...
Agent 在執行中判斷任務需要某個 skill 時，直接呼叫 `run_skill`。這是 LLM-initiated 路徑，與斜線版同 render 管線。

> Skill 啟用刻意設計為 **tool call**（lazy load），而非啟動時預先選擇 —— 避免為不需要 skill 的任務支付 skill body token。

### 同一對話多 Skill

同一對話可依序啟用多個 skill。每次 `run_skill` 在既有指令堆上追加；後啟用的 skill 透過 system prompt section 順序覆蓋或補充先前的。
...
| 觸發 | `MatchSkillCall` → run_skill | (a) daemon `runtime.SetRunner` 在 cron 觸發或 one-shot 到時呼叫、(b) TUI 手動 `/sched-<name>` |
...
4. 呼叫 `add_schedule` 綁定時間。

直呼 `add_schedule` 僅允許用於 **rebinding**（既存 scheduler skill 改時間）。
