...
呼叫 `list_tools` 取得工具清單，從結果中依任務需求選擇最合適的工具，確認必填參數。
...
呼叫 `write_file`：
- `path`：描述性檔名（`.py`，例如 `reminder.py`）
- `content`：步驟 3 的腳本
- `executable`：`true`

記下回傳訊息中的實際檔名（含 timestamp 後綴，例如 `reminder_1741569300.py`）。
...
若工具回傳包含「需啟動 app」警告，必須附加在 ID 行之後：
```
已設定排程，{時間描述}會{任務描述}。
-# ID: `{8 碼 ID}`
-# 排程已寫入檔案，但需啟動 app 才能實際執行。
```
