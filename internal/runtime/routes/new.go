...
func New() *gin.Engine {
	...
	handler.InitJarvisStatic()
	r.GET("/jarvis", handler.Jarvis())
	r.GET("/jarvis/page", handler.JarvisPage())
	r.GET("/jarvis/static/*filepath", handler.JarvisStatic())
	r.GET("/jarvis/src/*filepath", handler.JarvisSrc())
	r.GET("/jarvis/history", handler.JarvisHistory())
