...
func TestSend_GET(t *testing.T) {
	...
	result, err := Send(context.Background(), srv.URL+"/test", "GET", nil, nil, "json", 5)
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
...
func TestSend_POST_JSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
	...
	_, err := Send(context.Background(), srv.URL, "POST", nil, map[string]any{"key": "val"}, "json", 5)
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
}
...
func TestSend_CustomHeaders(t *testing.T) {
	...
	_, err := Send(context.Background(), srv.URL, "GET", map[string]string{"X-Custom": "value"}, nil, "", 5)
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
}
...
func TestSend_InvalidMethod(t *testing.T) {
	_, err := Send(context.Background(), "https://example.com", "TRACE", nil, nil, "", 5)
	if err == nil {
...
func TestSend_EmptyURL(t *testing.T) {
	_, err := Send(context.Background(), "", "GET", nil, nil, "", 5)
	if err == nil {
		t.Error("expected error for empty URL")
	}
}

func TestSend_TimeoutClamp(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
		w.Write([]byte("ok"))
	}))
	...
	_, err := Send(context.Background(), srv.URL, "GET", nil, nil, "", -10)
	if err != nil {
		t.Fatalf("Send with negative timeout: %v", err)
	}

	_, err = Send(context.Background(), srv.URL, "GET", nil, nil, "", 999)
	if err != nil {
		t.Fatalf("Send with large timeout: %v", err)
	}
}

func TestSend_POST_Form(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	_, err := Send(context.Background(), srv.URL, "POST", nil, map[string]any{"a": "b"}, "form", 5)
	if err != nil {
