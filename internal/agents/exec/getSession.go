...
func GetSession(execData ExecData) (*agentTypes.AgentSession, error) {
	...
	if overrideID := strings.TrimSpace(execData.SessionID); overrideID != "" {
		...
		session.SystemPrompts = BuildSystemPrompts(execData.WorkDir, execData.ExtraSystemPrompt, scanner, overrideID, execData.AllowAll, execData.WebMode)
	...
	case configExists:
		...
		session.SystemPrompts = BuildSystemPrompts(execData.WorkDir, execData.ExtraSystemPrompt, scanner, sessionID, execData.AllowAll, execData.WebMode)
